...
export default function StoffwechselFreebie() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('/api/subscribe-stoffwechsel', {
        method: 'POST',
      ...
      if (response.ok) {
        if (data.existingUser) setExistingUser(true)
        setSubmitted(true)
      } else {
  ...
  if (submitted) {
    return (
      <>
        <Head>
          <title>{existingUser ? 'Guide gesendet - Stoffwechsel Guide' : 'Bestätige deine E-Mail - Stoffwechsel Guide'}</title>
          ...
          >
            ...
            <p
              ...
              dangerouslySetInnerHTML={{
                __html: existingUser
                  ? `Dein kostenloser Stoffwechsel-Guide wurde an <strong>${formData.email}</strong> gesendet. Da du bereits in unserer Community bist, brauchst du deine E-Mail nicht mehr zu bestätigen.`
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-sm p-8 mb-8">
              ...
              <p className="text-gray-600 leading-relaxed">
                {existingUser
                  ...
                  : 'Klicke auf den Link in der E-Mail, um deine Anmeldung zu bestätigen. Danach erhältst du sofort Zugang zu deinem kostenlosen Stoffwechsel-Guide.'
                }
              </p>
            </div>
  ...
  return (
    <>
      <Head>
        <title>Kostenloser Stoffwechsel-Guide - Schnellstart für einen aktiven Stoffwechsel</title>
        <meta name="description" content="Lade dir jetzt deinen kostenlosen Schnellstart-Guide für einen aktiven Stoffwechsel herunter und starte deine Veränderung." />
            ...
            {mobileMenuOpen && (
              ...
              >
                <div className="flex flex-col space-y-4">
                  <Link href="/" className="text-sm text-gray-600 hover:text-gray-900 transition-colors" onClick={() => setMobileMenuOpen(false)}>Startseite</Link>
                  <Link href="/about" className="text-sm text-gray-600 hover:text-gray-900 transition-colors" onClick={() => setMobileMenuOpen(false)}>About me</Link>
                  <Link href="/login" className="text-sm text-gray-600 hover:text-gray-900 transition-colors" onClick={() => setMobileMenuOpen(false)}>Login</Link>
                  <Link href="/datenschutz" className="text-sm text-gray-600 hover:text-gray-900 transition-colors" onClick={() => setMobileMenuOpen(false)}>Datenschutz</Link>
                  <Link href="/impressum" className="text-sm text-gray-600 hover:text-gray-900 transition-colors" onClick={() => setMobileMenuOpen(false)}>Impressum</Link>
                </div>
              </motion.nav>
            )}
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-12 items-start">
            ...
            >
              ...
              <p className="text-lg text-gray-600 mb-8 leading-relaxed">
                Dein Stoffwechsel ist kein Feind, der gegen dich arbeitet. Er reagiert nur auf das, was du ihm täglich gibst.
                In diesem Guide erfährst du, wie du mit kleinen, bewussten Veränderungen deinen Stoffwechsel wirklich unterstützt –
                ohne Verzicht und ohne Diät-Stress.
              </p>
              ...
              <div className="space-y-6 mb-8">
                <div className="flex items-start">
                  ...
                  <div>
                    ...
                    <p className="text-gray-600">Praktische Impulse für deinen Alltag</p>
                  </div>
                </div>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-start">
                      ...
                      <label htmlFor="consent" className="ml-3 text-sm text-gray-600 leading-relaxed">
                        Ja, ich möchte den kostenlosen Stoffwechsel-Guide erhalten und bin einverstanden,
